...
// react-markdown v9 removed the `inline` prop from `code`.
// We differentiate inline vs block code by whether the parent is a `pre` tag.
// We do that by overriding `pre` to render a styled block, and `code` to render inline.
    ...
    <div className={className || undefined}>
      <ReactMarkdown
        ...
        components={{
          ...
          ol: ({ children }) => (
            <ol className="space-y-2 mb-4 ml-4 list-decimal marker:text-primary">
              {children}
            </ol>
          ),
          li: ({ children }) => (
          ...
          // Override `pre` for fenced code blocks (block code)
          pre: ({ children }) => (
          ...
          // `code` inside a `pre` is block; bare `code` is inline
          code: ({ node, children, ...props }: any) => {
            const isBlock = node?.position && node.data?.meta !== undefined
            // If parent is pre, just render a plain code tag — styled by `pre` above
            const parentType = (node as any)?.parent?.type
            if (parentType === 'element' && (node as any)?.parent?.tagName === 'pre') {
              return <code className="text-on-surface-variant">{children}</code>
            }
            // Inline code
            return (
